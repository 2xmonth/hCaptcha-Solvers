...
import numpy as np
import random
from curves._utils import isListOfPoints, isNumeric
from curves._beziercurve import BezierCurve
...
class HumanCurve():
    ...
    def generateCurve(self, **kwargs):
        ...
        """
        offsetBoundaryX = kwargs.get("offsetBoundaryX", 100)  # 100  NOT precision its the max it will go to
        offsetBoundaryY = kwargs.get("offsetBoundaryY", 100)  # 100
        ...
        knotsCount = kwargs.get("knotsCount", 2)  # 2, i think its the loops
        distortionMean = kwargs.get("distortionMean", 1)  # 1
        distortionStdev = kwargs.get("distortionStdev", 1)  # 1
        distortionFrequency = kwargs.get("distortionFrequency", 0.5)  # 0.5
        tween = kwargs.get("tweening", pytweening.easeOutQuad)
        targetPoints = kwargs.get("targetPoints", 125)  # 100
